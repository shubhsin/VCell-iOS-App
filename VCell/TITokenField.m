...
- (NSString *)description {
	return [NSString stringWithFormat:@"<TITokenFieldView %p; Token count = %d>", self, (int)self.tokenTitles.count];
}
...
- (void)didEndEditing {
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
            ...
            CGSize untokSize = [untokenized sizeWithAttributes:@{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:14]}];
			...
			if (_tokens.count > 1 && untokSize.width > availableWidth){
				untokenized = [NSString stringWithFormat:@"%d recipients", (int)titles.count];
			}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    NSDictionary *attribs = @{NSFontAttributeName: _font, NSParagraphStyleAttributeName: paragraphStyle};
    
    CGRect rect  = [_title boundingRectWithSize:CGSizeMake(_maxWidth - hTextPadding - accessoryWidth, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:attribs context:nil];
    CGSize titleSize = CGSizeMake(ceilf(rect.size.width), floorf(rect.size.height));
...
- (void)drawRect:(CGRect)rect {
	...
	CGPathRef outlinePath = newPathForRoundedRect(rect, self.bounds.size.height/10);
	...
	CGPathRef innerPath = newPathForRoundedRect(rect, self.bounds.size.height/10);
	...
	CGContextSaveGState(context);
	CGContextAddPath(context, innerPath);
    CGContextSetStrokeColor(context, (CGFloat[4]){200/255.0, 199/255.0, 204/255.0, 1});
    ...
    CGRect titleRect  = [_title boundingRectWithSize:CGSizeMake(_maxWidth - hTextPadding - accessoryWidth, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:attribs context:nil];
    CGSize titleSize = CGSizeMake(ceilf(titleRect.size.width), floorf(titleRect.size.height));
	...
	[_title drawInRect:textBounds withAttributes:attribs];
}
